...
import type { PendingModelRlsPolicy } from '@/features/model/apply-entire-model'
import type { TableKey } from '@/features/model/model-types'

type RlsPolicyInspectorSectionProps = {
  tableKey: TableKey
  pendingRlsPolicies: PendingModelRlsPolicy[]
  onChange: (next: PendingModelRlsPolicy[]) => void
}

export function RlsPolicyInspectorSection({
  tableKey,
  pendingRlsPolicies,
  onChange,
}: RlsPolicyInspectorSectionProps) {
  ...
  return (
    <div className="space-y-2">
      ...
      >
        <option value="create">{t("model.createUpdatePolicy")}</option>
        <option value="drop">{t("model.dropPolicy")}</option>
      </select>
      <Textarea
        className="min-h-24 font-mono text-[11px]"
        placeholder="ALTER TABLE ... ENABLE ROW LEVEL SECURITY; CREATE POLICY ..."
        value={sql}
      ...
      >
        Queue RLS policy change
      </Button>
      {pendingRlsPolicies.map((row) => (
        <div key={row.id} className="flex items-start gap-2 rounded border border-border/70 bg-muted/20 px-2 py-1.5">
          ...
          <button
            ...
            onClick={() => onChange(pendingRlsPolicies.filter((item) => item.id !== row.id))}
          >
