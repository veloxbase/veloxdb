...
use crate::db::{
    build_mysql_pool, build_mysql_pool_custom,
    build_pool, build_pool_custom, disconnect_connection,
    drop_pool, get_or_create_duckdb_connection, get_or_create_mongo_client,
    get_or_create_redis_client,
    load_connection, persist_connection_with_password, refresh_connection_pools,
    resolve_connection_engine, AppState, DEFAULT_MYSQL_PORT,
};
...
pub async fn connect_db(
    ...
    let ops = engines::get_engine(input.engine);
    ops.connect(&app, &state, &input, &connection_id).await.map_err(String::from)?;
...
pub async fn list_connections_command(app: AppHandle) -> Result<Vec<ConnectionSummary>, String> {
    crate::db::list_connections(&app).map_err(String::from)
}
...
pub async fn set_active_connection(
    ...
    let ops = engines::get_engine(stored_connection.engine);
    ops.ping(&app, &state, &connection_id).await.map_err(String::from)?;
...
pub async fn refresh_connection(
    ...
    refresh_connection_pools(&app, &state, &connection_id).await.map_err(String::from)
}
...
pub async fn rename_connection(
    ...
    crate::db::rename_connection_in_store(&app, &connection_id, &new_name).map_err(String::from)
}
...
pub async fn list_databases(
    ...
    ops.list_databases(&app, &state, &connection_id).await.map_err(String::from)
}
